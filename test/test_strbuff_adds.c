...
static void test_strbuff_adds_on_filled_buffer(void)
{
    ...
    TEST_ASSERT_EQUAL(str_size, strbuff_add(&buffer1, _rand_str2));
...
static void test_strbuff_adds_on_almost_full_buffer(void)
{
    ...
    TEST_ASSERT_EQUAL(free_space, strbuff_add(&buffer1, buffer2.str));
...
static void test_strbuff_adds_empty_str(void)
{
    ...
    TEST_ASSERT_EQUAL(0, strbuff_add(&buffer1, ""));
    TEST_ASSERT_EQUAL(_len, strbuff_len(&buffer1));
    TEST_ASSERT_EQUAL_STRING(_rand_str, buffer1.str);
}
...
{
    RUN_TEST(test_strbuff_adds_on_empty_buffer);
    RUN_TEST(test_strbuff_adds_on_filled_buffer);
    RUN_TEST(test_strbuff_adds_on_almost_full_buffer);
    RUN_TEST(test_strbuff_adds_empty_str);
}
