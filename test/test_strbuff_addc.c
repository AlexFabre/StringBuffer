...
static void test_strbuff_addc_on_empty_buffer(void)
{
    TEST_ASSERT_EQUAL(1, strbuff_add(&buffer1, 'A'));
    TEST_ASSERT_EQUAL(1, strbuff_len(&buffer1));
    TEST_ASSERT_EQUAL_STRING("A", buffer1.str);
}

static void test_strbuff_addc_on_filled_buffer(void)
{
    sprintf(buffer1.str, "Hello");

    TEST_ASSERT_EQUAL(1, strbuff_add(&buffer1, '!'));
    TEST_ASSERT_EQUAL(6, strbuff_len(&buffer1));
    TEST_ASSERT_EQUAL_STRING("Hello!", buffer1.str);
}

static void test_strbuff_addc_on_full_buffer(void)
{
...
{
    RUN_TEST(test_strbuff_addc_on_empty_buffer);
    RUN_TEST(test_strbuff_addc_on_filled_buffer);
    RUN_TEST(test_strbuff_addc_on_full_buffer);
    RUN_TEST(test_strbuff_addc_empty_char);
}
